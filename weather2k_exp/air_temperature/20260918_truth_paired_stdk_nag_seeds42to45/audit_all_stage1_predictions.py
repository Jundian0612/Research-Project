#!/usr/bin/env python3
"""Recompute pure-STDK outputs and compare all five seeds with Q's Stage 1."""
import csv
import hashlib
...
def main():
    spec = importlib.util.spec_from_file_location("stdk_five_seed_prediction_audit", MODEL_SOURCE)
        ...
        for scenario, (target, stations_key, start_time, end_time, suffix) in SCENARIOS.items():
            ...
            for i, time_index in enumerate(times):
                for j, station in enumerate(stations):
                    ...
                    point = float(predictions[i, j])
                    maximum = max(maximum, abs(point - q50))
                    squared_error_sum += (point - float(row["truth"])) ** 2
            recomputed_rmse = (squared_error_sum / len(rows)) ** 0.5
            pure_rmse = float(metadata["target_metrics"][target]["RMSE"])
            assert abs(recomputed_rmse - pure_rmse) < 1e-4
            assert maximum < 1e-4
            item = {
                "seed": seed, "scenario": scenario, "target": target,
                "pure_checkpoint_sha256": digest,
