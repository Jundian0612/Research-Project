...
新的 tuner 預設 output-dir：`weather2k_exp/air_temperature/qconvlstm_location_specific_tuning_20260915`。
...
```bash
mkdir -p weather2k_exp/air_temperature/qconvlstm_location_specific_tuning_20260915
set -o pipefail
PYTHONUNBUFFERED=1 python -u weather2k_exp/tune_stdk_qconvlstm_hyperparams.py \
  --stage all --seeds 41 42 \
  --output-dir weather2k_exp/air_temperature/qconvlstm_location_specific_tuning_20260915 \
  2>&1 | tee -a weather2k_exp/air_temperature/qconvlstm_location_specific_tuning_20260915/run.log
```
